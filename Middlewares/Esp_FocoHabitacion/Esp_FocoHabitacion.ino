...
const char* device_id = "foco_habitacion";

const char* relay_topic = "hotel/room/301/foco_habitacion/relay";
...
bool relayOn = true;           // Estado del relé
float acumulado = 0.0;         // Energía acumulada en Wh
const unsigned long intervaloMs = 2000;  // Intervalo de 2 segundos

// Callback para recibir órdenes del relé
...
void loop() {
  ...
  if (now - lastMsg >= intervaloMs) {
    ...
    if (relayOn) {
      // Simula una variación en torno a 10 W (entre 9.5 y 10.5 W)
      potenciaActual = random(95, 106) / 10.0;
